...
def user_details(backend, details, response, user=None, *args, **kwargs):
    ...
    if user:
        if kwargs['is_new']:
            ...
            # print url
            # avatar = urlopen(data).read()
            # print avatar
            data = requests.get('https://graph.facebook.com/v2.9/me?fields=id%2Cname%2Cemail&access_token={token}'.format(token=response['access_token']))
            print data.text
    else:
        pass
